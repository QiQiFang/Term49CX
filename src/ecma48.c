...
#define OSC_BUF_SIZE 8192
static char osc_buf[OSC_BUF_SIZE];
static int  osc_len = 0;
static char osc_overflow = 0;

/* SCS character sets: G0/G1 designations ('B' = US ASCII, '0' = DEC
 * Special Graphics) and the shift state selected by SI/SO */
static char charset[2] = {'B', 'B'};
static char charset_shift = 0;
static char scs_designator = '(';

/* DEC Special Graphics (ESC ( 0), code points 0x60..0x7e. The four
 * horizontal scan-line glyphs are approximated with U+2500 since most
 * fonts lack U+23BA..U+23BD. */
static const UChar dec_special_graphics[31] = {
...
/* xterm clears its pending-wrap ("hanging cursor") state on any control
 * that moves the cursor or edits the line. We represent that state as
 * buf->col == cols, so clamp back onto the screen before those ops --
 * otherwise the next printable lands one row down at column 0 and
 * multi-line redraws (e.g. zsh history recall) fall apart. Printable
 * characters and SGR must NOT clear it, so this is called only from the
 * cursor motion / editing handlers. */
static void ecma48_clear_wrap(){
  if(buf->col >= cols){
    buf->col = cols - 1;
  }
}
...
void ecma48_add_char(UChar c){
  ...
  if(writing_buffer == BUFFER_NORMAL){
    ...
    /* accumulate the OSC string (params and base64 are plain ASCII) */
    if(osc_len < OSC_BUF_SIZE - 1){
      osc_buf[osc_len++] = (c < 0x80) ? (char)c : '?';
    } else {
      osc_overflow = 1;
    }
  } /* else { BUFFER_DCS, etc. -> ignore for now } */
}

/* base64 decode in place; returns decoded length or -1 */
static int osc_base64_decode(char* s, int len){
  static const char* tbl = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  int out = 0, bits = 0, acc = 0, i;
  for(i = 0; i < len; ++i){
    const char* p;
    if(s[i] == '=' || s[i] == '\r' || s[i] == '\n'){
      continue;
    ...
    if(bits >= 8){
      bits -= 8;
      s[out++] = (char)((acc >> bits) & 0xff);
    }
  }
  return out;
}

/* handle a completed OSC string. Currently only OSC 52 (clipboard set)
 * is acted upon; clipboard queries ("?") are ignored for privacy. */
static void ecma48_process_osc(){
  if(osc_overflow || osc_len < 4){
    return;
  }
  osc_buf[osc_len] = '\0';
  if(strncmp(osc_buf, "52;", 3) == 0){
    char* payload = strchr(osc_buf + 3, ';');
    if(payload != NULL){
      ++payload;
      if(strcmp(payload, "?") != 0){
        int n = osc_base64_decode(payload, osc_len - (payload - osc_buf));
        if(n > 0){
          io_set_clipboard_bytes(payload, n);
        }
      }
    }
  }
...
int ecma48_bracketed_paste(){
  return modes.bracketed_paste;
}
...
*/
void ecma48_SO(){
  /* LS1: shift to the G1 character set */
  charset_shift = 1;
}
...
*/
void ecma48_SI(){
  /* LS0: shift to the G0 character set */
  charset_shift = 0;
}
    ...
    if(Pn[i] >= 0){
      ...
      switch (Pn[i]){
        ...
        case 10: // 10 primary (default) font
          /* font selection, not charset shifting: now that SI/SO are
           * implemented for SCS, mapping these to them (an old ACS hack)
           * would corrupt the shift state; xterm ignores 10-19 */
          break;
  ...
  for(i=0; i < NUM_ESCAPE_ARGS; ++i){
    if(Pn[i] >= 0){
      switch(Pn[i]){
        ...
        case 1000:
        case 1002:
        case 1003: modes.mouse_mode = Pn[i]; break;
        case 1001: break; /* highlight tracking not supported */
        case 1005: break; /* utf8 coordinates not supported */
  ...
  for(i=0; i < NUM_ESCAPE_ARGS; ++i){
    if(Pn[i] >= 0){
      switch(Pn[i]){
        ...
        case 1000:
        case 1002:
        case 1003: modes.mouse_mode = 0; break;
        case 1001:
        case 1005: break;
...
void setstate_SCS(char designator){
  /* remember which set is being designated: ( = G0, ) = G1,
   * anything else (%*+-./ for G2/G3 and encoding selection) is
   * consumed but ignored */
  scs_designator = designator;
  state = ECMA48_STATE_ANSI_SCS;
}

/* final byte of an SCS sequence, e.g. ESC ( 0 or ESC ( B */
void ecma48_SCS(char final){
  if(scs_designator == '('){
    charset[0] = final;
  } else if(scs_designator == ')'){
    charset[1] = final;
  }
  ecma48_end_control();
}
...
void dec_DECSTR(){
  ...
  cursor_shape = 0; // default cursor style
...
void dec_LED_ATTRIB(){
  /* CSI Ps SP q is DECSCUSR (set cursor style); plain CSI Ps q is the
   * DECLL LED control, which stays unimplemented */
  if(escape_args.ibyte == ' '){
    int Ps = escape_args.args[0][0] != '\0' ? (int)strtol(escape_args.args[0], NULL, 10) : 0;
    /* 0/1/2 block, 3/4 underline, 5/6 bar; blinking variants render steady */
    if(Ps >= 0 && Ps <= 6){
      cursor_shape = Ps;
    }
    ecma48_end_control();
    ...
    switch(state){
      ...
      case ECMA48_STATE_C1:
        switch(tbuf[i]){
          ...
          case 0x25: setstate_SCS((char)tbuf[i]); break;
          case 0x28: setstate_SCS((char)tbuf[i]); break;
          case 0x29: setstate_SCS((char)tbuf[i]); break;
          case 0x2a: setstate_SCS((char)tbuf[i]); break;
          case 0x2b: setstate_SCS((char)tbuf[i]); break;
          case 0x2d: setstate_SCS((char)tbuf[i]); break;
          case 0x2e: setstate_SCS((char)tbuf[i]); break;
          case 0x2f: setstate_SCS((char)tbuf[i]); break;
 ...
 * cb: 0=left, 1=middle, 2=right, 64/65=wheel up/down, +32 for motion.
 * press: 1 for press/motion/wheel, 0 for release.
 * col/row: 1-based screen cells. */
void ecma48_send_mouse_event(int cb, int press, int col, int row){
  ...
  if(modes.mouse_sgr){
    n = snprintf(out, sizeof(out), "\033[<%d;%d;%d%c", cb, col, row, press ? 'M' : 'm');
  } else {
    /* legacy X10 encoding; a release always reports button 3 */
    if(col > 222){ col = 222; }
    if(row > 222){ row = 222; }
    n = snprintf(out, sizeof(out), "\033[M%c%c%c",
                 (char)(32 + (press ? cb : 3)), (char)(32 + col), (char)(32 + row));
  }
  if(n > 0){
    io_write_master_char(out, n);
  }
}
