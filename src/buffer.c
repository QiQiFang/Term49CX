...
static int sel_anchor[2];  /* row, col */
static int sel_point[2];

void buf_sel_begin(int row, int col){
  sel_anchor[0] = sel_point[0] = row;
  sel_anchor[1] = sel_point[1] = col;
  sel_active = 1;
}

void buf_sel_update(int row, int col){
  if(!sel_active){
    buf_sel_begin(row, col);
    return;
  }
  sel_point[0] = row;
  sel_point[1] = col;
...
/* ordered range: start = min(anchor, point), end = max, reading order */
void buf_sel_get(int* r1, int* c1, int* r2, int* c2){
  if((sel_anchor[0] < sel_point[0]) ||
     (sel_anchor[0] == sel_point[0] && sel_anchor[1] <= sel_point[1])){
    *r1 = sel_anchor[0]; *c1 = sel_anchor[1];
    *r2 = sel_point[0];  *c2 = sel_point[1];
  } else {
    *r1 = sel_point[0];  *c1 = sel_point[1];
    *r2 = sel_anchor[0]; *c2 = sel_anchor[1];
  }
}

int buf_sel_contains(int row, int col){
  int r1, c1, r2, c2;
  if(!sel_active){
    return 0;
  }
...
/* keep the scrollback view inside the available history */
static void buf_clamp_view(){
  if(buf->view_offset > buf->top_line){
    buf->view_offset = buf->top_line;
  }
...
/* scroll the view: positive = back into history, negative = towards live */
void buf_scroll_view(int lines){
  buf->view_offset += lines;
  buf_clamp_view();
}
...
void buf_check_screen_scroll(){
  ...
  if(buf->line >= TEXT_BUFFER_SIZE - 1) {
    ...
    buf->line -= shift;
    buf_clamp_view();
    /* keep the selection glued to its (shifted) content */
    if(sel_active){
      sel_anchor[0] -= shift;
      sel_point[0] -= shift;
      if(sel_anchor[0] < 0 || sel_point[0] < 0){
        sel_active = 0;
      }
    }
  }
  ...
  if(buf->line - buf->top_line >= rows){
    ...
    int old_top = buf->top_line;
    buf->top_line = buf->line - (rows - 1);
    /* if the user is reading history, keep the view anchored on the
     * same content while new output scrolls underneath */
    if(buf->view_offset > 0){
      buf->view_offset += buf->top_line - old_top;
      buf_clamp_view();
    }
