...
pref_t *read_preferences(const char* filename) {
	...
	DEFAULT_LOOKUP(bool, config, "trackpad_enabled", prefs->trackpad_enabled, DEFAULT_TRACKPAD_ENABLED);
	DEFAULT_LOOKUP(int, config, "trackpad_sensitivity", prefs->trackpad_sensitivity, DEFAULT_TRACKPAD_SENSITIVITY);
	DEFAULT_LOOKUP(string, config, "trackpad_click_keys", prefs->trackpad_click_keys, DEFAULT_TRACKPAD_CLICK_KEYS);
	prefs->trackpad_click_keys = strdup(prefs->trackpad_click_keys);
	DEFAULT_LOOKUP(int, config, "scrollback_lines", prefs->scrollback_lines, DEFAULT_SCROLLBACK_LINES);
	DEFAULT_LOOKUP(string, config, "back_button_keys", prefs->back_button_keys, DEFAULT_BACK_BUTTON_KEYS);
	prefs->back_button_keys = strdup(prefs->back_button_keys);
...
void save_preferences(pref_t const* prefs, char const* filename) {
	...
	PREF_SET(root, setting, "trackpad_enabled", bool, BOOL, prefs->trackpad_enabled);
	PREF_SET(root, setting, "trackpad_sensitivity", int, INT, prefs->trackpad_sensitivity);
	PREF_SET(root, setting, "trackpad_click_keys", string, STRING, prefs->trackpad_click_keys);
	PREF_SET(root, setting, "scrollback_lines", int, INT, prefs->scrollback_lines);
	PREF_SET(root, setting, "back_button_keys", string, STRING, prefs->back_button_keys);
