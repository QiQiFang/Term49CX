...
/* trackpad (BlackBerry Classic): displacement counts per arrow keystroke
 * (lower = more sensitive) and keystrokes sent on trackpad click */
#define DEFAULT_TRACKPAD_ENABLED 1
#define DEFAULT_TRACKPAD_SENSITIVITY 60
#define DEFAULT_TRACKPAD_CLICK_KEYS "kent"

/* keystrokes sent by the hardware back button (Esc) */
#define DEFAULT_BACK_BUTTON_KEYS "\x1b"

/* extra scrollback history in lines (scroll with a touch drag or
 * alt + trackpad); 0 keeps only the historic two-screens buffer */
#define DEFAULT_SCROLLBACK_LINES 500
